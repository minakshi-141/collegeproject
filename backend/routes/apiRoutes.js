...
async function setupMulter() {
  const multer = (await import('multer')).default;
  const path = (await import('path')).default;
  
  const folder = "../public/resume"; // Ensure this path is correct and exists
  const storage = multer.diskStorage({
    destination: folder, // ensure this path exists
  ...
  }).single('resume'); // 'resume' is the name of the file input field

  // Define checkFileType function here or outside this function, depending on its dependencies
...
router.put("/user", jwtAuth, (req, res) => {
  ...
  const data = req.body;
  if (req.file) {
    data.resume = `/resume/${req.file.filename}`; // Add the path of the uploaded file to the data object
  }
  if (user.type == "recruiter") {
    Recruiter.findOne({ userId: user._id })
      .then((recruiter) => {
