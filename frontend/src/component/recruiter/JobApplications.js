...
const ApplicationTile = (props) => {
  ...
  const getResume = () => {
    ...
    // Ensure `application.jobApplicant.resume` contains the file name, e.g., 'Some_Important_questions.pdf'
    // Assuming `server` is something like 'http://localhost:4444'
    // Adjust the path according to how your server serves static files
    const url = `${server}/resume/${application.jobApplicant.resume}`;
    const urll = `${application.jobApplicant.resume}`;
    console.log("url",urll)
    console.log("Opening URL in new tab:", urll);
  
    // This attempts to open the PDF directly in a new tab
